...
def download_and_analyze(video_url):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            print(f"4. Thumbnail: {video_id}.webp")  # Add this line
            print(f"5. Subs: {video_id}.en.vtt")     # Changed from 4 to 5

            # D. Create video_input.json with file path
            video_input = {
                "video_file": os.path.abspath(f"{output_path}/{video_id}/{video_id}.mp4")
            }
            
            with open('video_info.json', 'w', encoding='utf-8') as f:
                json.dump(video_input, f, indent=2)
            
            print(f"5. Video Input: video_info.json")
